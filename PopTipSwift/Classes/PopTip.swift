...
open class PopTip: UIView {
  ...
  @objc open dynamic var popBorderColor = UIColor.clear
  /// The width for the poptip's border
  @objc open dynamic var popBorderWidth = CGFloat(0.0)
  /// The `Double` with the poptip's border radius
  @objc open dynamic var popCornerRadius = CGFloat(4.0)
  ...
  internal func setupVertically() -> (CGRect, CGPoint) {
    ...
    let constrainedArrowOffset = abs(arrowOffset) > (frame.size.width / 2) ? ((arrowOffset / arrowOffset) * (frame.size.width / 2 - popCornerRadius * 2)) : arrowOffset
    ...
    frame.size = CGSize(width: frame.width + popBorderWidth * 2, height: frame.height + popBorderWidth * 2)
  ...
  internal func setupHorizontally() -> (CGRect, CGPoint) {
    ...
    // Constraint the offset in the boundaries of the bubble, maintaining the sign (hence the arrowOffset / arrowOffset)
    let constrainedArrowOffset = abs(arrowOffset) > (frame.size.height / 2) ? ((arrowOffset / arrowOffset) * (frame.size.height / 2  - popCornerRadius * 2)) : arrowOffset
    // Make sure that the bubble doesn't leave the boundaries of the view
  ...
  fileprivate func setup() {
    ...
    case .right:
      ...
      layer.position = CGPoint(x: layer.position.x + rect.width / 2, y: layer.position.y + rect.height * anchor)
    case .none:
      rect.size = CGSize(width: textBounds.width + padding * 2.0 + edgeInsets.horizontal + popBorderWidth * 2, height: textBounds.height + padding * 2.0 + edgeInsets.vertical + popBorderWidth * 2)
  ...
  open override func draw(_ rect: CGRect) {
    if isRounded {
      ...
      popCornerRadius = (frame.size.height - (showHorizontally ? 0 : arrowSize.height)) / 2
    }

    let path = PopTip.pathWith(rect: rect, frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: arrowPosition, arrowRadius: arrowRadius, popBorderWidth: popBorderWidth, radius: popCornerRadius)
    ...
    popBorderColor.setStroke()
    path.lineWidth = popBorderWidth
    path.stroke()
  ...
  fileprivate func updateBubble() {
    stopActionAnimation {
      UIView.animate(withDuration: 0.2, delay: 0, options: [.transitionCrossDissolve, .beginFromCurrentState], animations: {
        ...
        let path = PopTip.pathWith(rect: self.frame, frame: self.frame, direction: self.direction, arrowSize: self.arrowSize, arrowPosition: self.arrowPosition, arrowRadius: self.arrowRadius, popBorderWidth: self.popBorderWidth, radius: self.popCornerRadius)
